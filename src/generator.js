...
const TEMPLATES = [
    ...
    { s: "Maths", t: "Logarithms", d: "Hard", q: "Value of $\\log \\tan(1^\\circ) + \\log \\tan(2^\\circ) + \\dots + \\log \\tan(89^\\circ)$ is:", a: "0", o: ["1", "45", "90"] },
    ...
    // --- MATHS: TRIGONOMETRY ---
    { s: "Maths", t: "Trigonometry", d: "Easy", q: "The value of $\\sin^2\\theta + \\cos^2\\theta$ is:", a: "1", o: ["0", "-1", "2"] },
    { s: "Maths", t: "Trigonometry", d: "Medium", q: "The value of $\\tan(45^\\circ)$ is:", a: "1", o: ["0", "Infinity", "$1/\\sqrt{2}$"] },
    { s: "Maths", t: "Trigonometry", d: "Hard", q: "The period of $\\sin(x)$ is:", a: "$2\\pi$", o: ["$\\pi$", "$\\pi/2$", "$3\\pi/2$"] },
    ...
    { s: "Maths", t: "Vectors", d: "Hard", q: "Projection of vector $\\vec{a}$ on vector $\\vec{b}$ is:", a: "$\\frac{\\vec{a} \\cdot \\vec{b}}{|\\vec{b}|}$", o: ["$\\frac{\\vec{a} \\cdot \\vec{b}}{|\\vec{a}|}$", "$\\vec{a} \\cdot \\vec{b}$", "$|\\vec{a}| \\cdot |\\vec{b}|$"] },
    ...
    { s: "Physics", t: "Units & Measurements", d: "Medium", q: "Dimensional formula for Work is:", a: "$[ML^2T^{-2}]$", o: ["$[MLT^{-2}]$", "$[ML^2T^{-1}]$", "$[MLT^{-1}]$"] },
    ...
    { s: "Physics", t: "Heat", d: "Medium", q: "Specific heat capacity of water is (in SI):", a: "4200 J/kg K", o: ["2100 J/kg K", "420 J/kg K", "1000 J/kg K"] },
];
...
export function generateDailyBatch() {
    ...
    const getUniqueQ = (subject, difficulty) => {
        let attempt = 0;
        while (attempt < 50) { // Increased attempts for higher quality
            const candidate = getQ(subject, difficulty);
            
            // Quality Filter (Phase 5)
            const meaningless = [/^pm\d+$/i, /^[a-z]{1,3}\d?$/i, /cdot/i, /times/i];
            const options = [candidate.option_a, candidate.option_b, candidate.option_c, candidate.option_d];
            
            const isMalformed = options.some(o => meaningless.some(p => p.test(o.trim())));
            const hasValue = options.some(o => /\d/.test(o) || o.includes('$') || o.trim().length > 5);

            if (!generatedQuestions.has(candidate.question) && !isMalformed && hasValue) {
                generatedQuestions.add(candidate.question);
                return candidate;
            }
            attempt++;
        }
        // Fallback: force unique and ensure it doesn't look like pm2
        const fallback = getQ(subject, difficulty);
        fallback.question += ` [ID:${Math.floor(Math.random()*1000)}]`;
        return fallback;
    };
